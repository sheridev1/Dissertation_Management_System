...
if (isset($_POST['submit'])) {
    ...
    $sql = "SELECT * FROM `projects` WHERE supervisorId=" . $_SESSION['id'] . " and title='" . $_POST['title'] . "'";
    ...
    if ($result->num_rows > 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $foundPid = $row['pid'];
                $sql = "SELECT * FROM `projects` inner join supervisor ON projects.supervisorId=supervisor.id where pid=" . $foundPid;
                    ...
                    if ($insert) {
                        ...
                        if ($prjFileFlag == 1) {
                            if ($members == 'Individual') {
                                ...
                                $sql = "INSERT INTO `projects`( `supervisorId`, `title`, `discipline`, `area`, `status`, `supervisor`, `website`, `overview`, `details`, `approve`, `type`, `startdate`, `enddate`,`type2`,`members`,`scope`,`projectfile`,`studentid`) VALUES (" . $_SESSION['id'] . ",'" . $_POST['title'] . "','" . $_POST['discipline'] . "','" . $_POST['area'] . "',1,'" . $_SESSION['id'] . "','" . $_POST['website'] . "','" . $_POST['overview'] . "','" . $_POST['details'] . "',1,'" . $_POST['type'] . "','" . $_POST['sdate'] . "','" . $_POST['edate'] . "','" . $type2 . "','" . $members . "','" . $scope . "','" . $targetFilePath . "','" . $_POST['members'] . "'');";
                                $con->query($sql); 
                                ...
                                $sql = "INSERT INTO `projects`( `supervisorId`, `title`, `discipline`, `area`, `status`, `supervisor`, `website`, `overview`, `details`, `approve`, `type`, `startdate`, `enddate`,`type2`,`members`,`scope`,`projectfile`) VALUES (" . $_SESSION['id'] . ",'" . $_POST['title'] . "','" . $_POST['discipline'] . "','" . $_POST['area'] . "',1,'" . $_SESSION['id'] . "','" . $_POST['website'] . "','" . $_POST['overview'] . "','" . $_POST['details'] . "',1,'" . $_POST['type'] . "','" . $_POST['sdate'] . "','" . $_POST['edate'] . "','" . $type2 . "','" . $members . "','" . $scope . "','" . $targetFilePath . "');";
                                if ($con->query($sql) == true) {
                                    $sql = "select * from projects where supervisorId=" . $_SESSION['id'] . " and title ='" . $title . "'";
                                    $result = $con->query($sql);
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            $pid = $row['pid'];
                ...
                <div class="row">
                    ...
                    <div class="col-xl-2" id='sm5'>
                        <div class="submit-field">
                            <h5>Members</h5>
                            <select 
                            class="selectpicker with-border" name="Member" data-size="7" title="Students">
                            <?php
