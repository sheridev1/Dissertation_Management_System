...
if (isset($_POST['supervisor'])) {
    ...
    $scope = $_POST['scope'];
    $sql = "UPDATE `projects` SET `title`='" . $title . "',`discipline`='" . $discipline . "',`area`='" . $area . "',`studentid`='" . $_POST['members'] . "',`supervisor`='" . $supervisor . "',`website`='" . $website . "',`overview`='" . $overview . "',`details`='" . $details . "',`type`='" . $type . "',`startdate`='" . $sdate . "',`enddate`='" . $edate . "',`type2`='" . $type2 . "',`members`='" . $members . "',`scope`='" . $scope . "' WHERE pid=" . $_POST['pid'];
...
if (isset($_POST['update_Screenshots'])) {
    ...
    $sql = "select * from images where usprj='" . '_' . $prnm . "'";
    $result = $con->query($sql) or die($con->error);

    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            unlink('../projects/' . '_' . $prnm . '_' . $row['src']);
        }
    }
    $sql = "delete from images where usprj='" . '_' . $prnm . "'";
    $con->query($sql) or die($con->error);
    ...
    if (!empty(array_filter($_FILES['files']['name']))) {
        ...
        foreach ($_FILES['files']['name'] as $key => $val) {
            ...
            $targetFilePath = $targetDir . '_' . $_POST['prjnm'] . '_' . $fileName;
            ...
            if (in_array($fileType, $allowTypes)) {
                ...
                try {
                    ...
                    $insertValuesSQL .= "('" . '_' . $_POST['prjnm'] . "','" . $fileName . "'),";
...
if (isset($_POST['update_projectFile'])) {
    ...
    if (!empty($_FILES['files1']['name'])) {
        ...
        $targetFilePath = $targetDir .  '_' . $prnm . '_' . $fileName;
        // Check whether file type is valid
        $fileType = pathinfo($targetFilePath, PATHINFO_EXTENSION);
        $targetFilePath = $targetDir  . '_' . $prnm . '.' . $fileType;
        if (in_array($fileType, $allowTypes)) {
            // Upload file to server

            try {
                if (move_uploaded_file($_FILES["files1"]["tmp_name"][0], $targetFilePath)) {
                    $targetFilePath =  '_' . $prnm . '.' . $fileType;
            ...
            <form method="post" enctype="multipart/form-data" action="#">
                <div class="content with-padding padding-bottom-10">
                    <div class="row">
                        ...
                        <div class="submit-field">
                            ...
                            <select class="selectpicker with-border" name="supervisor" title="Super Visor">                       
                                ...
                                <option selected value="<?php echo $_SESSION['id']; ?>"><?php echo $_SESSION['fullname']?>
                                </option>
                        ...
                        <div class="col-xl-4=2">
                            <div class="submit-field">
                                <h5>Members *</h5>
                                <select class="selectpicker with-border"  id="membersSelect"  name="members" data-size="7" title="Members" required>
                                    <option value='Individual' <?php if ($type != 'Individual' && $type != 'Group Base')
                                        echo 'selected'; ?>>
                                        Individual </option>
                                    <option value='Group Base' <?php if ($type == 'Group Base')
                                        echo 'selected'; ?>> Group
                                        Base </option>
                                </select>
                            </div>
                        </div>
                        <div class="col-xl-2" id='sm5'>
                        <div class="submit-field">
                            <h5>Members</h5>
                            <select class="selectpicker with-border" name="members" data-size="7" title="Students" selected <?php echo 'value='.$members;?> >
                            <?php echo $members;
                            ...
                            while ($row = mysqli_fetch_array($query)) {
                                ...
                                <option value="<?php echo $row['id']; ?>" <?php if($members==$row['id']) echo 'selected'; ?> ><?php echo $row['fname'] . ' ' . $row['lname'];?>
                                </option>
                    ...
                    <div class="col-xl-3 " id='sm1'>
                        <div class="submit-field">
                            <h5>1st Member </h5>
                            <select class="selectpicker with-border" name="mem1" data-size="7" title="Supervisor">
...
function handleMemberTypeChange() {
    ...
    const sm5 = document.getElementById('sm5');
