...
if(isset($_POST['usapp'])){
  if($_POST['status'] ==1){
	...
	$us= $_POST['usapp'];
$sql = "update `student` set active=".$ustatus." where  username='".$us."' ";
$con->query($sql);
}
if(isset($_POST['usdel'])){

	$us= $_POST['usdel'];
$sql = "delete from `student` where  username='".$us."' ";
...
$sql ="select * from student where approve=1 order by active desc";
...
<div class='row'>
    <div class="col-xl-12">
        <div class="dashboard-box margin-top-0">
            ...
            <!-- Headline -->
            <div class="headline">
                <h3><i class="icon-material-outline-business-center"></i>Students</h3>
            </div>
...
<?php if($row['active']==0){
        ...
        <?php if($row['active']==1){
          ?><a href="#" class="button red ripple-effect " title="Active" onclick="status('<?php echo $row['username']; ?>', '0');">Deactive</a><?php
        }
        else {
          ?><a href="#" class="button green ripple-effect " title="Deavtive" onclick="status('<?php echo $row['username']; ?>', '1');">Active</a><?php
        } ?>
...
<script>
  ...
  function submitForm(userid, fromprj) {
    // Create a dynamic form element
    const form = document.createElement('form');
    form.action = 'viewuser.php'; // Replace 'viewuser.php' with the actual URL where you want to post the data
    form.method = 'post';
