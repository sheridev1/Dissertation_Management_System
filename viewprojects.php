...
while( $row = $result->fetch_assoc() ) {
    ...
    <h3 class = 'job-listing-title'><a href = ''><?php echo $row[ 'title' ] ?></a><?php   if ( $row[ 'approve' ] == 1 ) {
        echo '<span class="dashboard-status-button green">Approved</span>';
    } else {
        echo '<span class="dashboard-status-button red">Not Approved</span>';
    }
    ...
    <a onclick = "action(<?php echo $row['pid']; ?>)" class = 'button ripple-effect' style = 'color:white' class = 'btn-default'><i class = 'icon-material-outline-supervisor-account'></i>Feedbacks</a>
